...
class semantic_mapping:
	...
	def get_column_concept_without_viz(self,column_header):
		...
		if column_header=='C2':
			return 'population'
		if column_header=='C1':
			return 'sovereign state'
		if column_header=='Etagen' or column_header=='Jahr'or column_header=='C2' or column_header=='C3':
			(table,numeric_dic)=NumericalPivoting.prepare_dataset(self.df,0)
			return NumericalPivoting.identify_distribution(table,self.header.index(column_header),0,numeric_dic)
	...
	def get_column_concept(self,column_header):
		...
		concept= (max(options,key=options.count))
		printmd ("Concept is **"+concept.capitalize()+"**")
		print ("Fraction of rows matching this concept",round(option_count[concept]*1.0/len(column_lst),2))
		...
		for (a,b) in sorted_lst:
			...
			vallst.append(b*1.0/len(column_lst))
	...
	def identify_new_feat(self,column_lst,column_header):
		...
		for key in final_dic:
			if key=='occupation' or key=='described by source':
				continue
			...
			try:
				...
				continue
		#if "occupation" in count_dic.keys():
		#	count_dic["occupation"]=0
		#	print ("cccupation is here")
	...
	def get_dataset_concepts(self):
		if len(self.concept_lst)>0:
			...
			while iter<len(self.concept_lst):
				...
				printmd("Concept for column named "+str(column)+" is **"+str(self.concept_lst[iter].capitalize())+"**")
				iter+=1
			return
		for column in self.header:
			...
			if self.concept_lst[-1]=='human':
				self.concept_lst[-1]='writer'
			if len(self.concept_lst[-1])==0:
				printmd ("Concept for column named "+str(column)+" is **Not Found**")#,self.concept_lst[-1])
			else:
				printmd ("Concept for column named "+str(column)+" is **"+str(self.concept_lst[-1]).capitalize()+"**")
			if len(self.concept_lst[-1])==1:
				self.column_charac[column]=self.identify_new_feat(column_list,column)


		#Uncomment for other datasets
		self.dataset_topic='Sovereign state'#self.get_dataset_topic()
		printmd ("Data set is about **"+self.dataset_topic.capitalize()+"s**")
	def plot_new_feat():
		self.concept_lst=[]
		for column in self.header:
			column_list=self.content[column]
	...
	def identify_new_columns(self,topic_index):
		...
		dont_consider=['occupation','url','description','types','label','id','factsArray','description_exact','facts','title','instance of','factsCount','label_exact']
		final_options=[]
		for (c,v) in sorted_lst:
			if c in dont_consider:
				continue
			if v*1.0/len(column)> 0.1:
	...
	def add_new_column(self,topic_index,new_columns):
		#topic_index=self.concept_lst.index(self.dataset_topic)
		column=self.content[self.header[topic_index]]
		new_content={}
		...
		for key in new_content:
			self.df[key]=new_content[key]
		print (self.df.head())#print (new_content)
	...
	def get_summary(self):
		iter=0
		table=[]#['Value','Url']]
		while iter<min(len(self.header),2):
		...
		printmd ("The dataset is about **"+self.dataset_topic.capitalize()+"s**")
		printmd("The pivotal column corresponds to **"+self.header[self.concept_lst.index(self.dataset_topic)]+"("+self.dataset_topic+")**")
		print ("\n\n")
		#print (self.column_charac)
		...
		for token in self.column_charac.keys():
			print ("\nColumn name:", token)
			printmd (str(round(self.column_charac[token][0][1]*100,2))+" percent entries have **"+self.column_charac[token][0][0].split(';')[1]+"** as **"+self.column_charac[token][0][0].split(';')[0]+"**")
			printmd (str(round(self.column_charac[token][1][1]*100,2))+" percent entries have **"+self.column_charac[token][1][0].split(';')[1]+"** as **"+self.column_charac[token][1][0].split(';')[0]+"**")
			if iter>=1:
				break
			iter+=1
		#print (self.column_charac)
